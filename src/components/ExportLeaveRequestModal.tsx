...
export default function ExportLeaveRequestModal({
  isOpen,
  onClose,
  factoryId,
}: Props) {
  ...
  const handleExport = async () => {
    setError(null);
    setLoading(true);
    showLoading('Đang export tổng hợp nghỉ phép...');

    try {
      const blob = await leaveRequestApi.exportXLSX(factoryId, selectedYear, selectedMonth);
      ...
      const filename = `tong-hop-nghi-phep-${selectedYear}-${selectedMonth.toString().padStart(2, '0')}.xlsx`;
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 pt-4">
            ...
            >
              ...
              {loading ? 'Đang xuất file...' : 'Xuất tổng hợp nghỉ phép'}
            </button>
          </div>
